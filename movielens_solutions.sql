...
CREATE VIEW movieratings AS SELECT r.id, r.user_id, r.movie_id, r.rating, r.rated_at, m.title, m.release_date FROM
movies m JOIN ratings r ON m.id = r.movie_id;
...
SELECT * FROM genres g JOIN counts c ON g.id=c.genre_id;
